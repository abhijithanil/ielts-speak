...
const Practice = () => {
  ...
  useEffect(() => {
    if (isRecording && recordingTimeRemaining > 0) {
  ...
  }, [isRecording, recordingTimeRemaining]);
  ...
  const handleTimeUp = () => {
    ...
    if (!hasAnswered) {
      ...
      setTimeout(() => {
        setError(null);
        // Properly reset recording state before moving to next question
        resetRecording();
        nextQuestion();
      }, 2000);
    } else {
  ...
  const loadSectionQuestions = async (section) => {
    ...
    if (data.type === 'cue_card') {
      ...
      if (section === 'part1') {
        const maxQuestions = Math.min(questionList.length, Math.floor(Math.random() * 3) + 4);
        limitedQuestions = questionList.slice(0, maxQuestions);
      } else if (section === 'part3') {
        const maxQuestions = Math.min(questionList.length, Math.floor(Math.random() * 3) + 5);
        limitedQuestions = questionList.slice(0, maxQuestions);
      }
  ...
  const loadCompleteTest = async () => {
    ...
    if (data.part1 && Array.isArray(data.part1)) {
      ...
      const maxPart1Questions = Math.min(part1Questions.length, Math.floor(Math.random() * 3) + 4);
    ...
    if (data.part3 && Array.isArray(data.part3)) {
      ...
      const maxPart3Questions = Math.min(part3Questions.length, Math.floor(Math.random() * 3) + 5);
  ...
  const nextQuestion = () => {
    ...
    if (currentQuestionIndex < questions.length - 1) {
      ...
      // Reset all recording and timer state
      setAudioBlob(null);
      setAudioUrl(null);
      setRecordingTime(0);
      ...
      // Stop preparation and auto-recording timers
      if (preparationTimer) {
        clearInterval(preparationTimer);
        ...
        <div className="bg-gradient-to-br from-white to-blue-50 rounded-2xl shadow-2xl p-4 sm:p-8 border border-blue-100">
          <div className="text-center space-y-4 sm:space-y-6">
            <div className="flex items-center justify-center space-x-2 sm:space-x-3">
              <h2 className="text-2xl sm:text-3xl font-bold text-gray-900">Practice Complete!</h2>
              <button
                onClick={() => {
              ...
              >
                <Volume2 className={`h-4 w-4 text-gray-600 ${isSpeaking ? 'animate-bounce' : ''}`} />
              </button>
            </div>
      ...
      <div className="bg-gradient-to-br from-white to-gray-50 rounded-2xl shadow-xl p-4 sm:p-8 mb-4 sm:mb-8 border border-gray-200">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 sm:mb-6 space-y-4 sm:space-y-0">
          ...
          <div className="flex flex-wrap items-center gap-2 sm:gap-3">
            <div className="bg-white px-2 sm:px-4 py-2 rounded-xl shadow-md">
              <div className="flex items-center space-x-2">
            ...
            <div className="bg-white px-2 sm:px-4 py-2 rounded-xl shadow-md">
              <span className="text-xs sm:text-base text-gray-700 font-semibold">
        ...
        <div className="w-full bg-gray-200 rounded-full h-2 sm:h-3 mb-4 sm:mb-6 overflow-hidden">
          <div 
            className="bg-gradient-to-r from-blue-500 via-purple-500 to-pink-500 h-2 sm:h-3 rounded-full transition-all duration-500 ease-out shadow-lg"
          ...
          <div className="relative z-10">
            ...
            <div className="bg-white bg-opacity-10 rounded-xl p-4 sm:p-6 backdrop-blur-sm">
              <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between mb-3 sm:mb-4">
                <p className="text-sm sm:text-lg leading-relaxed font-medium flex-1 mb-3 sm:mb-0 sm:mr-4">{currentQuestion}</p>
                <button
                  onClick={() => {
                    handleUserInteraction();
                    isSpeaking ? stopSpeaking() : speakQuestion(currentQuestion);
                  }}
                  className={`flex-shrink-0 p-2 sm:p-3 rounded-full transition-all duration-300 self-start ${
                    isSpeaking 
                      ? 'bg-red-500 hover:bg-red-600 animate-pulse' 
                      : 'bg-white bg-opacity-20 hover:bg-opacity-30'
                  }`}
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center space-y-3 sm:space-y-0 sm:space-x-4">
            {!isRecording && !audioBlob && (
              <button
                ...
                className="flex items-center justify-center px-6 sm:px-8 py-3 sm:py-4 bg-gradient-to-r from-red-500 to-red-600 text-white rounded-2xl hover:from-red-600 hover:to-red-700 transition-all duration-300 shadow-lg hover:shadow-xl transform hover:scale-105 w-full sm:w-auto"
              >
            ...
            {isRecording && (
              <button
                ...
                className="flex items-center justify-center px-6 sm:px-8 py-3 sm:py-4 bg-gradient-to-r from-gray-500 to-gray-600 text-white rounded-2xl hover:from-gray-600 hover:to-gray-700 transition-all duration-300 shadow-lg hover:shadow-xl transform hover:scale-105 w-full sm:w-auto"
              >
            ...
            {audioBlob && !isRecording && (
              <div className="flex items-center space-x-3 px-4 sm:px-6 py-3 bg-blue-100 text-blue-700 rounded-xl w-full sm:w-auto justify-center">
            ...
            {audioBlob && !isRecording && !isAnalyzing && (
              <button
                ...
                className="flex items-center justify-center px-4 sm:px-6 py-3 bg-gradient-to-r from-green-500 to-green-600 text-white rounded-xl hover:from-green-600 hover:to-green-700 transition-all duration-300 shadow-lg hover:shadow-xl w-full sm:w-auto"
              >
            ...
            {config.allowSkip && !hasAnswered && !isRecording && (
              <button
                ...
                className="flex items-center justify-center px-4 sm:px-6 py-3 bg-gradient-to-r from-yellow-500 to-yellow-600 text-white rounded-xl hover:from-yellow-600 hover:to-yellow-700 transition-all duration-300 shadow-lg hover:shadow-xl w-full sm:w-auto"
              >
